...
use eframe::{
    egui::{
        CentralPanel, Hyperlink, ScrollArea, Separator, TextStyle, TopBottomPanel, Ui, Vec2,
        Visuals,
    },
    epi::App,
    ...
    if let Ok(response) = NewsAPI::new(&api_key).fetch() {
        ...
        for a in resp_articles.iter() {
            ...
            if let Err(e) = news_tx.send(news) {
                tracing::error!("发送新闻数据发生错误: {}", e);
            }
        }
    } else {
        tracing::error!("获取新闻失败");
    }
}

fn render_footer(ctx: &eframe::egui::CtxRef) {
    TopBottomPanel::bottom("footer").show(ctx, |ui| {
        ui.vertical_centered(|ui| {
            ui.add_space(10.);
            // ui.add(Label::new("数据源来自: ").text_style(TextStyle::Monospace));
            ui.add(Hyperlink::new("https://newsapi.org").text("数据源来自 newsapi"));
            ui.add(
